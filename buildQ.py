...
class QBuilder(object):
	...
	def getQValue(self, s, a):
		s = (int(round(s[0])), int(round(s[1])), int(round(s[2])), int(round(s[3])), int(round(s[4])))
		a = int(round(a))		
	...
	def learnFromDataSARSA(self, data, num_iters = 100, alpha=0.5, gamma=0.7):
		...
		for k in range(num_iters):
			for i in range(data.shape[0]-2):
				s_t[0], s_t[1], s_t[2], s_t[3], s_t[4],a_t, r_t, s_p[0], s_p[1], s_p[2], s_p[3] , s_p[4] = data.values[i]
				_, _,_,_, a_t1, _,_, _,_, _ = data.values[i+1]
				s_t_key = (int(round(s_t[0])), int(round(s_t[1])), int(round(s_t[2])), int(round(s_t[3])), int(round(s_t[4])))
	...
	def learnFromDataQLearning(self, data, num_iters=100, alpha=0.5, gamma=0.7):
		...
		for k in range(num_iters):
			...
			pass
		pass
	# def learnFromDataSARSAlamb(self, data, num_iters = 100, alpha=0.5, gamma=0.7):
	...
	# 			a_t_key = int(round(a_t))
	# 			if (s_t_key, a_t_key) in N:
	# 				N[(s_t_key, a_t_key)] += 1
	# 			else:
	# 				N[(s_t_key, a_t_key)] = 1
	# 			delta = (r_t + gamma*self.getQValue(s_p, a_t1) - self.getQValue(s_t, a_t))
